...
object Day5 extends App {
  ...
  def solutionPart1(instructions: Seq[Int]): Int = {
    ...
    def executeInstruction(steps: Int, index: Int, currentInstructions: Vector[Int]): Int = {
      ...
      if (nextIndex >= currentInstructions.length || nextIndex < 0) steps + 1
        ...
        steps + 1,
        nextIndex,
        currentInstructions.updated(index, currentInstructions(index) + 1)
      )
    }
    executeInstruction(0, 0, instructions.toVector)
  }
    ...
    def executeInstruction(steps: Int, index: Int, currentInstructions: Vector[Int]): Int = {
      ...
      if (nextIndex >= currentInstructions.length || nextIndex < 0) steps + 1
        ...
        val updatedInstruction =
          if (currentInstructions(index) >= 3) currentInstructions(index) - 1
          else currentInstructions(index) + 1
        executeInstruction(
          steps + 1,
          nextIndex,
          currentInstructions.updated(index, updatedInstruction)
        )
      }
    }
    executeInstruction(0, 0, instructions.toVector)
  ...
  val problemInput = Source.fromResource("day5.txt").getLines.map(_.toInt).toList
